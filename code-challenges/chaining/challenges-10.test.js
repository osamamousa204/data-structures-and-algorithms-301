...
const count = (target, input) => {
    let counter = input.reduce((acc,val)=>{
...
const divisibleByFiveTwoToThePower = (input) => {
    let remove = input.map((val)=>{
        let remove2 = val.filter((val2)=>{
            if (isNaN(val2) || val2 % 5 != 0) {
                return false
            }else{return true}
        })
        return remove2
      })
    let power = remove.map(val=>{
      let power2 =  val.map(val2=>{             
            return Math.pow( 2 , val2)
        })
        return power2
    })
    return power
