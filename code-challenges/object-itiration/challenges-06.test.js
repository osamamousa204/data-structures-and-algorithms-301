...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(val => {
    houses.push(Object.values(val)[3])
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let names = [];
  arr.forEach(val => {
    names.push(Object.values(val)[0])
  });
  let child = [];
  arr.forEach(val => {
    child.push(Object.values(val)[2])
  });

  for (let i = 0 ; i <= names.length ; i++){
    if(character == names[i] && child[i] != []){
      return true
    }else if(character !== names[i] && child[i] == []) {
      return false
    }
  }
...
const hasChildrenEntries = (arr, character) => {
  ...
  // let firstName = [];
  // arr.forEach((val,idx) => {
  //   names.push(Object.entries(val))
  //    firstName.push(names[idx * 4][2])
  // });

  // let child = [];
  // let firstchild = [];
  // arr.forEach((val,idx) => {
  //   child.push(Object.entries(val))
  //   firstchild.push(names[idx * 4 + 2][2])
  // });

  // for (let i = 0 ; i <= firstName.length ; i++){
  //   if(character == firstName[i] && firstchild[i] != []){
  //     return true
  //   }else if(character !== firstName[i] && firstchild[i] == []) {
  //     return false
  //   }
  // }
};
