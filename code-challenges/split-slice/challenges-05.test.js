...
const listFoods = (recipe) => {
 ...
 recipe.ingredients.forEach((val,idx) => {
     ...
     result[idx] = val.slice(idx1 + 1)
    let idx2 = result[idx].indexOf(' ');
     result[idx] = result[idx].slice(idx2 + 1)
 });
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  let splited = [];
  recipe.ingredients.forEach((val,idx) => {
    splited[idx] = val.split(' ')
    splited[idx] = splited[idx].slice(2);
    splited[idx] = splited[idx].join(' ')
    result[idx] = splited[idx]
});
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach((val,idx) => {
    splited[idx] = val.split(' ')
    splited[idx] = splited[idx].slice(0,1);
    splited[idx] = splited[idx].join(' ')
    result[idx] = splited[idx]
});
  return result;
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if (numberOfCharacters > str.length){
      ...
      let cut1 = str.slice(-numberOfCharacters);
      let cut2 = str.split(cut1)
      return cut2[0]
